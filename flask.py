...
def findusername():
    ...
    #recieve the shortcode (username)
    shortcode = request.form['shortcode']
    
    #recieve the server (returns server)
    server = request.form['server']
    
    #api_keys: create the api_url based on username
    api_key = "RGAPI-58eaa969-3786-40c9-98cf-2ce44254afac"
    ...
    #extracts second api
    #only extract ranked if the player has ranked
    ##need to fix
    
    player_tier = player_info2[0]["tier"]
    player_rank = player_info2[0]["rank"]
    player_wins = str(player_info2[0]["wins"]) #int
    player_losses = str(player_info2[0]["losses"]) #int
